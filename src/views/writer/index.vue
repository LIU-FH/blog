      ...
      <div class="w-full flex justify-between">
        ...
        <div class="flex items-center">
          <div class="flex items-center mr-5">
            <md-button @click="onPrev" class="md-icon-button">
              <i class="icon-fanhui text-lg" />
            </md-button>
            <p>{{page}}</p>
            <md-button @click="onNext" class="md-icon-button">
              <i class="icon-fangxiangyou text-lg" />
            </md-button>
          </div>
      ...
      <md-table class="flex-1 max-height-none" v-model="list" md-sort="name" md-sort-order="asc">
        <md-table-row slot="md-table-row" slot-scope="{item}">
          ...
          <md-table-cell md-label="展示图">
            <div
              class="bg-local bg-cover bg-center bg-no-repeat h-20 w-32 rounded"
          ...
          <md-table-cell md-label="状态" md-sort-by="status">{{ item.status ? "开启":"关闭" }}</md-table-cell>
...
export default {
  ...
  methods: {
    ...
    onNext() {
      if (this.page < this.writerListData.meta.last_page) {
        this.page = this.page + 1;
        this.loadData();
      }
    },
